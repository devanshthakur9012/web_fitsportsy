...
<nav class="navbar rounded m-0 bg-white z-30 shadow-md relative">
    <div class="flex xl:space-y-0 xxsm:space-y-2 xmd:space-y-0 md:space-y-2 msm:flex-col md:flex-col xmd:flex-row xxsm:flex-col lg:flex-wrap xmd:flex-wrap justify-between
                                        ...
                                        @foreach ($category as $item)
                                            <div class="flex items-left justify-left">
                                                <a href="{{ url('/events-category/' . $item->id . '/' . $item->name) }}"
                                                    class="flex items-left text-base font-poppins font-normal leading-5 {{ $catactive == $item->name ? ' text-primary capitalize p-2 bg-primary-light' : ' ' }}  capitalize p-2 hover:bg-primary-light rounded-md w-full">{{ $item->name }}+1</a>
                                            </div>
                                        @endforeach
                                    @endif
                                    <div class="flex items-left justify-left">
