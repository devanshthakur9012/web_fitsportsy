...
class ApiController extends Controller
{
    ...
    public function events(Request $request)
    {
        ...
        $from = 0;
        $to = 2000;
        $event= Event::select('events.name as name','events.id','events.image','temple_name')->where([['events.status', 1], ['events.is_deleted', 0], ['event_status', 'Pending']])->where(function($q) use($date){
            $q->where('events.end_time', '>', $date)->orWhere('event_type',2);
        });
        ...
        $events = $event->orderBy('event_type','ASC')->orderBy('events.start_time', 'desc')->simplePaginate(20);
        return response()->json(['msg' => null, 'data' => $events, 'success' => true], 200);
    }
    ...
    public function eventDetail($id)
    {
        ...
        $imagesArr = explode(",", $data->gallery);
        $images = [];
        foreach($imagesArr as $a){
            $images[] = [
                'img'=>asset("images/upload/".$a)
            ];
        }
